...
substituicoes = {
    ...
    "local_prestacao": "São Paulo/SP (Foro eleito)",
    "condicao_pagamento_dias": "Vencimento até o dia 15 do mês subsequente.",
}
for item in auditoria:
    if item["campo"] in substituicoes:
        item["valor"] = substituicoes[item["campo"]]
    if item["campo"] == "local_prestacao":
        item.update({
            "pagina": "10",
            "clausula_secao": "Cláusula 10.22 — Foro",
            "trecho_evidencia": "As Partes elegem o foro da capital do estado de São Paulo como o único competente para dirimir conflitos.",
            "confianca": 100,
        })
...
--- PÁGINA 10 OCR ---
10.22. As Partes elegem o foro da capital do estado de São Paulo como o único competente para dirimir quaisquer controvérsias decorrentes deste Contrato.
--- PÁGINA 11 OCR ---
...
assert resultado["condicao_pagamento_dias"] == "15DD"
assert resultado["local_prestacao"] == "Não localizado com segurança"
linha_local = next(i for i in resultado["auditoria_campos"] if i["campo"] == "local_prestacao")
assert linha_local["status"] == "NÃO_LOCALIZADO"
assert "foro eleito" in linha_local["trecho_evidencia"].lower()
...
assert 90 <= resultado["score"] < 100
assert 90 <= resultado["confianca_extracao"] < 100

print("OK - V9: 15DD, foro rejeitado como local de prestação e 1 campo realmente ausente")
