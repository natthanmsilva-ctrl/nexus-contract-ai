# =========================================================
# Auditor de Contratos - Grupo SBF - VERSÃO REVISADA
...
def _montar_resumo_aditivos(aditivos: List[Dict[str, Any]]) -> str:
    ...
    total = len(aditivos)
    assinados = sum(1 for a in aditivos if str(a.get("Assinado", "")).strip().upper() == "SIM")
    nao_assinados = sum(1 for a in aditivos if str(a.get("Assinado", "")).strip().upper() == "NÃO")

    valor_total = 0.0
    achou_valor = False

    for aditivo in aditivos:
        valor = _parse_moeda_brasil(aditivo.get("Valor do aditivo"))
    ...
    if achou_valor:
        resumo += f". Valor total dos aditivos identificados: {_formatar_moeda_brasil(valor_total)}."
    else:
        resumo += ". Nenhum valor específico de aditivo foi localizado com clareza."
...
def aplicar_regras_aditivos(base: Dict[str, Any], texto: str) -> Dict[str, Any]:
    """Consolida aditivos em lista/tabela e mantém compatibilidade com campos antigos."""
    aditivos = []
    ...
    # Compatibilidade com possíveis nomes alternativos da IA
    if not aditivos:
        for chave in ["aditivos", "aditivos_identificados", "termos_aditivos", "lista_aditivos"]:
            if isinstance(base.get(chave), list) and base.get(chave):
                aditivos = normalizar_aditivos_contrato(base.get(chave))
                break

    # Fallback com campos antigos que já existiam no robô
    if not aditivos:
        campos_antigos = [
            base.get("data_assinatura_aditivo"),
            base.get("aditivo_escopo"),
            base.get("aditivo_quitacao"),
            base.get("aditivo_valor"),
            base.get("aditivo_prazo"),
            base.get("quantidade_renovacoes"),
        ]

        if any(_valor_informado(c) for c in campos_antigos):
            aditivos = normalizar_aditivos_contrato([{
                "numero_aditivo": "1",
                "tipo_aditivo": "Aditivo identificado",
                "anexo_origem": "Não localizado",
                "data_aditivo": base.get("data_assinatura_aditivo", "Não localizado"),
                "data_carga_robo": base.get("data_analise", "Não localizado"),
                "assinado": "Não localizado",
                "data_assinatura_aditivo": base.get("data_assinatura_aditivo", "Não localizado"),
                "pessoas_que_assinaram_aditivo": "Não localizado",
                "valor_aditivo": base.get("aditivo_valor", "Não localizado"),
                "impacto_valor": base.get("aditivo_valor", "Não localizado"),
                "impacto_prazo": base.get("aditivo_prazo", "Não localizado"),
                "periodo_vigencia_aditivo": base.get("aditivo_prazo", "Não localizado"),
                "objeto_escopo_aditivo": base.get("aditivo_escopo", "Não localizado"),
                "itens_aditivo": [],
                "status_validacao_aditivo": "Aditivo localizado parcialmente nos campos legados do robô.",
                "observacoes_aditivo": base.get("aditivo_quitacao", "Não localizado"),
            }])

    data_carga = base.get("data_analise")
    if not _valor_informado(data_carga):
        data_carga = datetime.now().strftime("%d/%m/%Y %H:%M")
    for aditivo in aditivos:
        if not _valor_informado(aditivo.get("Data de carga no robô")):
            aditivo["Data de carga no robô"] = data_carga

    aditivos = _preencher_anexo_aditivo_por_texto(aditivos, texto)
    ...
    aditivos = _enriquecer_aditivos_por_anexo(aditivos, texto, base)
...
def normalizar(resultado: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if itens_servico_percentual and (not itens_ia or _itens_sao_apenas_atributos_comerciais(itens_ia)):
...
def render_aditivos_contrato(resultado: Dict[str, Any]) -> None:
    """Renderiza seção executiva de aditivos identificados, com tabela compacta e detalhes em expansor."""
    ...
    total = len(aditivos)
    assinados = sum(1 for a in aditivos if str(a.get("Assinado", "")).strip().upper() == "SIM")
    nao_assinados = sum(1 for a in aditivos if str(a.get("Assinado", "")).strip().upper() in ("NÃO", "NAO"))

    valor_total_aditivos = 0.0
    tem_valor = False
    for aditivo in aditivos:
        v = _parse_moeda_brasil(aditivo.get("Valor do aditivo"))
        if v is not None:
            valor_total_aditivos += v
            tem_valor = True

    valor_total_fmt = _formatar_moeda_brasil(valor_total_aditivos) if tem_valor else "Não identificado"

    c1, c2, c3, c4 = st.columns(4)
    c1.markdown(render_metric("Aditivos", total), unsafe_allow_html=True)
    c2.markdown(render_metric("Assinados", assinados), unsafe_allow_html=True)
    c3.markdown(render_metric("Não assinados", nao_assinados), unsafe_allow_html=True)
    c4.markdown(render_metric("Valor dos aditivos", valor_total_fmt), unsafe_allow_html=True)
    ...
    # Tabela principal compacta: só o essencial
    colunas_resumo = [
        "Nº",
        "Anexo do aditivo",
        "Data do aditivo",
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "Anexo do aditivo": st.column_config.TextColumn("Anexo", width="large"),
            "Data do aditivo": st.column_config.TextColumn("Data"),
            "Assinado": st.column_config.TextColumn("Assinado"),
            "Data da assinatura": st.column_config.TextColumn("Assinatura"),
            "Valor do aditivo": st.column_config.TextColumn("Valor"),
            "Impacto no prazo": st.column_config.TextColumn("Impacto prazo", width="medium"),
            "Status de validação": st.column_config.TextColumn("Status", width="large"),
        },
    )
    ...
    st.dataframe(
        ...
        column_config={
            "Nº": st.column_config.TextColumn("Nº", width="small"),
            "Tipo do aditivo": st.column_config.TextColumn("Tipo", width="medium"),
            "Anexo do aditivo": st.column_config.TextColumn("Anexo", width="large"),
            "Data do aditivo": st.column_config.TextColumn("Data aditivo"),
            "Data de carga no robô": st.column_config.TextColumn("Carga robô"),
            "Assinado": st.column_config.TextColumn("Assinado"),
            "Quem assinou": st.column_config.TextColumn("Quem assinou", width="large"),
            "Valor do aditivo": st.column_config.TextColumn("Valor"),
            "Impacto no valor": st.column_config.TextColumn("Impacto valor", width="large"),
            "Impacto no prazo": st.column_config.TextColumn("Impacto prazo", width="large"),
            "Escopo do aditivo": st.column_config.TextColumn("Escopo", width="large"),
            "Itens do aditivo": st.column_config.TextColumn("Itens", width="large"),
            "Status de validação": st.column_config.TextColumn("Status", width="large"),
        },
    )
    ...
    for idx, aditivo in enumerate(aditivos, 1):
        itens = aditivo.get("_itens_aditivo", [])
        if itens:
            with st.expander(f"📦 Itens do aditivo {aditivo.get('Nº', idx)}"):
                st.dataframe(
                    pd.DataFrame(itens),
                    use_container_width=True,
                    hide_index=True,
    ...
    if not arquivos:
        ...
        if st.button("🚀 Analisar contrato e anexos", use_container_width=True):
            with st.spinner("Lendo contrato e anexos..."):
                ...
                if itens_percentuais and (not resultado.get("itens_contrato") or _itens_sao_apenas_atributos_comerciais(resultado.get("itens_contrato", []))):
                ...
                # Reprocessa aditivos depois que data_analise e arquivos_analisados já existem
